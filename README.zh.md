...
设置页面板——路由 / 自适应 / 指示灯三开关、实时统计芯片、🏆 可用性排行（后验均值 × 延迟惩罚）、供应商分组折叠台账（展开可编辑到期日 / 用量 / 质量档）：

![穷鬼路由器设置页面板](docs/panel.png)

输入框左侧的 ⚡ 改道指示灯——实时显示最近一笔请求的实际去向与改道原因（如 `aux-explore` 探索调度），悬停看详情，超十分钟自动变灰：

![输入框改道指示灯](docs/badge.png)
