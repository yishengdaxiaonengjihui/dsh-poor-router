...
export async function apply(ctx, config = {}) {
    ...
    const rpcOff = []
    log('poor-router v22 applied; routing=' + (ROUTE.enabled ? 'ON' : 'OFF') + '; adaptive=TS; paid-guard=ON; tier-ladder=S>A>B>C(manual); aux-explore=1/6; ttft-gate=30s; text-me=ON; badge-toggle=ON; hourly-v2+7d-prune=ON; switch-log=50; provcool-persist=ON; soft-fuse=30s~5min-escalating');
    return () => {
