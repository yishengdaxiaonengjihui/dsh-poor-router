...
export async function apply(ctx, config = {}) {
    ...
    // 免费候选全灭时的最后手段：只允许 escapeHatch 且当日未超上限的付费模型，选最便宜的
    ...
    function pickAlt(curEntry, excludeProvider) {
      ...
      const cands = (pool.entries || []).filter(e => e.id !== excludeId && !e.disabled && !e.paid && !(e._cool > now)
        ...
        && !(excludeProvider && provCool[excludeProvider] > now)
        && !congested(e.id));
      if (!cands.length) return null;
    ...
    ctx.on('agent/request-error', (payload, next) => {
      ...
      try {
        ...
        if (HARD.test(sig)) {
          ...
          probeLog('soft fail ' + payload.provider + ' (内置重试接管) :: ' + sig.slice(0, 60));
        }
      ...
      const def = {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'string',
              enum: ['setHealthy', 'setUnhealthy', 'setExpiry', 'setGrant', 'setCooldown', 'resetCounters', 'setDisabled', 'setEnabled', 'setRouting', 'setAdaptive', 'setBadge', 'setTier', 'clearProviderCooldown', 'reloadPool'],
            },
            id: { type: 'string' },
            value: {
              type: 'object',
    ...
    const rpcOff = []
    log('poor-router v21 applied; routing=' + (ROUTE.enabled ? 'ON' : 'OFF') + '; adaptive=TS; paid-guard=ON; tier-ladder=S>A>B>C(manual); aux-explore=1/6; ttft-gate=30s; text-me=ON; badge-toggle=ON; hourly-v2+7d-prune=ON; switch-log=50; provcool-persist=ON');
    return () => {
