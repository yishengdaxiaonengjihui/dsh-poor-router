  ...
  factory: (require) => {
    ...
    function Dashboard() {
      ...
      const headBar = E('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 10 } },
        ...
        E('button', { style: Object.assign({}, btn, { color: sum.paidAllowed === true ? '#ef4444' : '#22c55e', borderColor: sum.paidAllowed === true ? '#ef4444' : undefined }), disabled: busy, title: '付费逃生舱总闸：关闭时免费全灭也绝不动付费模型，宁可失败不花钱', onClick: () => withBusy(() => act('setPaidAllowed', 'global', { allowed: sum.paidAllowed !== true }).then(load)) },
          '● 付费逃生 ' + (sum.paidAllowed === true ? '已授权' : '封禁')),
